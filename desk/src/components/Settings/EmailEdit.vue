...
const state = reactive({
  ...
  dontmanage_mail_site: props.accountData?.dontmanage_mail_site || "",
...
const info = {
  ...
  link: "https://docs.dontmanageerp.com/docs/user/manual/en/email-account",
};
...
const isDirty = computed(() => {
  return (
    ...
    state.dontmanage_mail_site !== props.accountData.dontmanage_mail_site
  );
});

async function callRenameDoc() {
  const d = await call("dontmanage.client.rename_doc", {
...
async function callSetValue(values) {
  const d = await call("dontmanage.client.set_value", {
    doctype: "Email Account",
