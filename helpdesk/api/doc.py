...
def get_list_data(
    ...
    rows = dontmanage.parse_json(rows or "[]")
    columns = dontmanage.parse_json(columns or "[]")
    filters = dontmanage.parse_json(filters or "[]")
    ...
    if columns or rows:
        is_default = False
        is_custom = True
        columns = dontmanage.parse_json(columns)
        rows = dontmanage.parse_json(rows)
    ...
    fields = dontmanage.get_meta(doctype).fields
    ...
    return {
        ...
        "total_count": dontmanage.get_list(
...
def get_filterable_fields(doctype: str, show_customer_portal_fields=False):
    ...
    enable_restrictions = dontmanage.db.get_single_value(
...
def handle_default_view(doctype, _list, show_customer_portal_fields):
    [columns, rows] = dontmanage.get_value(
        "HD View",
        {
            "is_default": 1,
            "user": dontmanage.session.user,
...
def handle_at_me_support(filters):
    ...
    for key in filters:
        ...
        if isinstance(value, list):
            if "@me" in value:
                value[value.index("@me")] = dontmanage.session.user
            elif "%@me%" in value:
                index = [i for i, v in enumerate(value) if v == "%@me%"]
                for i in index:
                    value[i] = "%" + dontmanage.session.user + "%"
        elif value == "@me":
            filters[key] = dontmanage.session.user
...
def handle_team_restrictions(filters):
    ...
    if not enable_restrictions:
        return
    show_tickets_without_team = dontmanage.db.get_single_value(
    ...
    teams = (
        ...
        .where(QBTeamMember.user == dontmanage.session.user)
        .join(QBTeam)
