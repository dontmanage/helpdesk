	...
	preset_filters = list(
		(
			...
			.where(
				(
					(
						(fd_preset_filter.type == "User") & (fd_preset_filter.user == dontmanage.session.user)
					)
...
def get_field_data_type(doctype, fieldname):
	...
	field = dontmanage.get_meta(doctype).get_field(fieldname)
...
def get_filtered_select_field_options(doctype, fieldname, query):
	...
	if field.fieldtype == "Select":
		...
		dontmanage.throw("Select options are only available for Select fields")
...
def save_filter_preset(doctype, is_global, title, filters):
	...
	preset_filter = dontmanage.get_doc(
		{
