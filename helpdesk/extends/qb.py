...
VERSION = dontmanage.__version__
...
def get_query(*args, **kwargs):
	...
	if not VERSION.startswith("15"):
		query = dontmanage.qb.engine.get_query(*args, **kwargs)

		# DontManage framework version below 15 (as of this) does not support
		# order by for `get_query`. Hence it is needed to manually add it
		# to our query
		table = kwargs.get("table")
		order_by_field, order_by_dir = extract_order_by(kwargs)
		QBTable = dontmanage.qb.DocType(table)
		query = query.orderby(
	...
	return dontmanage.qb.get_query(*args, **kwargs)
