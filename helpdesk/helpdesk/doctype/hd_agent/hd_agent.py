	...
	def add_to_support_rotations(self, group=None):
		...
		if not group:
			...
			rule_docs.append(
				dontmanage.get_doc(
					"Assignment Rule",
					dontmanage.get_doc("HD Settings").get_base_support_rotation(),
				)
			)
			...
			if self.groups:
				for group in self.groups:
					try:
						team_assignment_rule = dontmanage.get_doc(
							"HD Team", group.team
						).get_assignment_rule()
						rule_docs.append(
							dontmanage.get_doc(
			...
			if next(
				...
				rule_docs.append(
					dontmanage.get_doc(
						"Assignment Rule",
						dontmanage.get_doc("HD Team", group).get_assignment_rule(),
					)
				)
			else:
				dontmanage.throw(
		...
		if group:
			...
			for group in self.groups:
				rule_docs.append(
					dontmanage.get_doc(
						"Assignment Rule",
						dontmanage.get_doc("HD Team", group.team).get_assignment_rule(),
					)
				)
