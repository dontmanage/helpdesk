...
class HDArticleCategory(Document):
	def before_save(self):
		if self.idx == -1 and self.status == "Published":
			...
			self.idx = cint(
				dontmanage.db.count("HD Article Category", {"parent_category": self.parent_category})
			)
	...
	def get_breadcrumbs(self):
		...
		while current_category.parent_category:
			current_category = dontmanage.get_doc("HD Article Category", current_category.parent_category)
			breadcrumbs.append(
