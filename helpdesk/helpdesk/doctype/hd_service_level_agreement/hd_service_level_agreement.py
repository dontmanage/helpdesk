...
class HDServiceLevelAgreement(Document):
	...
	def check_priorities(self):
		...
		if not len(set(priorities)) == len(priorities):
			...
			dontmanage.throw(_("Priority {0} has been repeated.").format(repeated_priority))
		...
		try:
			...
			dontmanage.throw(_("Select a Default Priority."))
	...
	def check_support_and_resolution(self):
		...
		if not len(set(support_days)) == len(support_days):
			...
			dontmanage.throw(_("Workday {0} has been repeated.").format(repeated_days))

	def validate_doc(self):
		if (
			self.enabled
			and self.document_type == "HD Ticket"
			and not dontmanage.db.get_single_value(
				"HD Settings", "track_service_level_agreement"
			)
		):
			dontmanage.throw(
				_("{0} is not enabled in {1}").format(
					dontmanage.bold("Track HD Service Level Agreement"),
					get_link_to_form("HD Settings", "HD Settings"),
				)
			)

		if self.default_sla and dontmanage.db.exists(
			...
			dontmanage.throw(_("Default HD Service Level Agreement for {0} already exists.").format(self.document_type))
		...
		if (
			self.entity_type
			and self.entity
			and dontmanage.db.exists(
				"HD Service Level Agreement",
				{"entity_type": self.entity_type, "entity": self.entity, "name": ["!=", self.name]},
			)
		):
			dontmanage.throw(
				_("HD Service Level Agreement for {0} {1} already exists.").format(
					dontmanage.bold(self.entity_type), dontmanage.bold(self.entity)
				)
			)

	def validate_status_field(self):
		meta = dontmanage.get_meta(self.document_type)
		if not meta.get_field("status"):
			dontmanage.throw(
				_("The Document Type {0} must have a Status field to configureHD Service Level Agreement").format(
					dontmanage.bold(self.document_type)
				)
			)

	def validate_condition(self):
		temp_doc = dontmanage.new_doc(self.document_type)
		if self.condition:
			try:
				dontmanage.safe_eval(self.condition, None, get_context(temp_doc))
			except Exception:
				dontmanage.throw(_("The Condition '{0}' is invalid").format(self.condition))

	def get_hd_service_level_agreement_priority(self, priority):
		priority = dontmanage.get_doc(
	...
	def before_insert(self):
		# no need to set up HD Service Level Agreement fields for Ticket dt as they are standard fields in Ticket
		if self.document_type == "HD Ticket":
			return

		hd_service_level_agreement_fields = get_hd_service_level_agreement_fields()
		meta = dontmanage.get_meta(self.document_type, cached=False)

		if meta.custom:
			self.create_docfields(meta, hd_service_level_agreement_fields)
		else:
			self.create_custom_fields(meta, hd_service_level_agreement_fields)

	def on_trash(self):
		if self.service_level == "Default":
			dontmanage.throw("The Default HD Service Level Agreement cannot be deleted")
		set_documents_with_active_hd_service_level_agreement()
	...
	def create_docfields(self, meta, hd_service_level_agreement_fields):
		last_index = len(meta.fields)

		for field in hd_service_level_agreement_fields:
		...
		dontmanage.get_doc("DocType", self.document_type).save(ignore_permissions=True)

	def create_custom_fields(self, meta, hd_service_level_agreement_fields):
		for field in hd_service_level_agreement_fields:
			if not meta.has_field(field.get("fieldname")):
				dontmanage.get_doc(
					{
	...
	def reset_field_properties(self, field, field_dt, hd_service_level_agreement_field):
		field = dontmanage.get_doc(field_dt, {"fieldname": field.fieldname})
		field.label = hd_service_level_agreement_field.get("label")
		field.fieldname = hd_service_level_agreement_field.get("fieldname")
		field.fieldtype = hd_service_level_agreement_field.get("fieldtype")
		field.collapsible = hd_service_level_agreement_field.get("collapsible")
		field.hidden = hd_service_level_agreement_field.get("hidden")
		field.options = hd_service_level_agreement_field.get("options")
		field.read_only = hd_service_level_agreement_field.get("read_only")
		field.hidden = hd_service_level_agreement_field.get("hidden")
		field.description = hd_service_level_agreement_field.get("description")
		field.default = hd_service_level_agreement_field.get("default")
		field.save(ignore_permissions=True)


def check_agreement_status():
	hd_service_level_agreements = dontmanage.get_all(
	...
	for hd_service_level_agreement in hd_service_level_agreements:
		doc = dontmanage.get_doc("HD Service Level Agreement", hd_service_level_agreement.name)
		if doc.end_date and getdate(doc.end_date) < getdate(dontmanage.utils.getdate()):
			dontmanage.db.set_value("HD Service Level Agreement", hd_service_level_agreement.name, "enabled", 0)
...
def get_active_hd_service_level_agreement_for(doc):
	...
	default_sla = dontmanage.get_all(
		"HD Service Level Agreement",
	...
	for agreement in agreements:
		...
		if not condition or (
			condition and dontmanage.safe_eval(condition, None, get_context(doc))
		):
...
def get_context(doc):
	return {
		"doc": doc.as_dict(),
		"nowdate": nowdate,
		"dontmanage": dontmanage._dict(utils=get_safe_globals().get("dontmanage").get("utils")),
	}
...
def get_hd_service_level_agreement_filters(doctype, name):
	...
	filters = [
		["HD Service Level Agreement", "document_type", "=", doctype],
		["HD Service Level Agreement", "enabled", "=", 1],
	]
	...
	return {
		"priority": [
			priority.priority
			for priority in dontmanage.get_all(
				"HD Service Level Priority", filters={"parent": name}, fields=["priority"]
			)
		],
		"hd_service_level_agreements": [
			d.name for d in dontmanage.get_all("HD Service Level Agreement", filters=filters, or_filters=or_filters)
		],
	}
...
def get_documents_with_active_hd_service_level_agreement():
	hd_service_level_agreement_doctypes = dontmanage.cache().hget("hd_service_level_agreement", "active")

	if hd_service_level_agreement_doctypes is None:
		return set_documents_with_active_hd_service_level_agreement()

	return hd_service_level_agreement_doctypes


def set_documents_with_active_hd_service_level_agreement():
	active = [hd_service_level_agreement.document_type for hd_service_level_agreement in dontmanage.get_all("HD Service Level Agreement", fields=["document_type"])]
	dontmanage.cache().hset("hd_service_level_agreement", "active", active)
	return active


def apply(doc, method=None):
	# Applies HD Service Level Agreement to document on validate
	if (
		dontmanage.flags.in_patch
		or dontmanage.flags.in_migrate
		or dontmanage.flags.in_install
		or dontmanage.flags.in_setup_wizard
		or doc.doctype not in get_documents_with_active_hd_service_level_agreement()
	):
		return

	hd_service_level_agreement = get_active_hd_service_level_agreement_for(doc)

	if not hd_service_level_agreement:
		return

	process_hd_service_level_agreement(doc, hd_service_level_agreement)


def process_hd_service_level_agreement(doc, hd_service_level_agreement):
...
def handle_status_change(doc, apply_sla_for_resolution):
	now_time = dontmanage.flags.current_time or now_datetime(doc.get("owner"))
	prev_status = dontmanage.db.get_value(doc.doctype, doc.name, "status")
...
def get_fulfillment_statuses(hd_service_level_agreement):
	return [
		entry.status
		for entry in dontmanage.db.get_all(
			"HD Service Level Agreement Fulfilled On Status", filters={"parent": hd_service_level_agreement}, fields=["status"],
		)
	]


def get_hold_statuses(hd_service_level_agreement):
	return [
		entry.status
		for entry in dontmanage.db.get_all(
...
def get_allotted_seconds(parameter, service_level):
	...
	if parameter == "response":
		...
		dontmanage.throw(_("{0} parameter is invalid").format(parameter))
...
def get_support_days(service_level):
	...
	for service in service_level.get("support_and_resolution"):
		support_days[service.workday] = dontmanage._dict(
...
def change_hd_service_level_agreement_and_priority(self):
	if (
		...
		and dontmanage.db.exists("HD Ticket", self.name)
		and dontmanage.db.get_single_value(
			"HD Settings", "track_service_level_agreement"
		)
	):

		if not self.priority == dontmanage.db.get_value("HD Ticket", self.name, "priority"):
			self.set_response_and_resolution_time(priority=self.priority, hd_service_level_agreement=self.hd_service_level_agreement)
			dontmanage.msgprint(_("Priority has been changed to {0}.").format(self.priority))

		if not self.hd_service_level_agreement == dontmanage.db.get_value("HD Ticket", self.name, "hd_service_level_agreement"):
			self.set_response_and_resolution_time(priority=self.priority, hd_service_level_agreement=self.hd_service_level_agreement)
			dontmanage.msgprint(_("HD Service Level Agreement has been changed to {0}.").format(self.hd_service_level_agreement))


def get_response_and_resolution_duration(doc):
	hd_service_level_agreement = dontmanage.get_doc("HD Service Level Agreement", doc.hd_service_level_agreement)
	priority = hd_service_level_agreement.get_hd_service_level_agreement_priority(doc.priority)
	priority.update(
		{
			"support_and_resolution": hd_service_level_agreement.support_and_resolution,
			"holiday_list": hd_service_level_agreement.holiday_list,
		}
	)
	return priority
...
def reset_hd_service_level_agreement(doc, reason, user):
	if not dontmanage.db.get_single_value("HD Settings", "allow_resetting_service_level_agreement"):
		dontmanage.throw(_("Allow Resetting HD Service Level Agreement from Helpdesk Settings."))
...
def on_communication_update(doc, status):
	...
	for_resolution = dontmanage.db.get_value("HD Service Level Agreement", parent.hd_service_level_agreement, "apply_sla_for_resolution")
...
def record_assigned_users_on_failure(doc):
	...
	if assigned_users:
		...
		message = _("First Response HD Service Level Agreement Overdue by {}").format(assigned_users)
		doc.add_comment(comment_type="Assigned", text=message)


def get_hd_service_level_agreement_fields():
	return [
		{
			"collapsible": 1,
		...
		{"fieldname": "hd_service_level_agreement", "fieldtype": "Link", "label": "HD Service Level Agreement", "options": "HD Service Level Agreement",},
		{
			"fieldname": "priority",
			"fieldtype": "Link",
			"label": "Priority",
		...
		{"fieldname": "cb", "fieldtype": "Column Break", "read_only": 1},
		{
			...
			"fieldtype": "Select",
			"label": "HD Service Level Agreement Status",
...
def get_tz(user):
	return dontmanage.db.get_value("User", user, "time_zone") or get_system_timezone()
...
@dontmanage.whitelist()
def get_hd_service_level_agreement_doctypes():
	doctypes = []
	data = dontmanage.get_list("HD Service Level Agreement", {"enabled": 1}, ["document_type"], distinct=1)
