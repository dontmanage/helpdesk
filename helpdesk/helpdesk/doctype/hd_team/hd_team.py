...
from dontmanage.core.doctype.version.version import get_diff
...
class HDTeam(Document):
    ...
    def after_insert(self):
        ...
        assignment_rule_doc = dontmanage.get_doc("Assignment Rule", self.assignment_rule)
    ...
    def after_rename(self, olddn, newdn, merge=False):
        ...
        rule_doc = dontmanage.get_doc("Assignment Rule", rule)
    ...
    def update_support_rotations(self):
        ...
        """
        assg_rule_doc = dontmanage.get_doc("Assignment Rule", self.assignment_rule)
    ...
    def update_assignment_rule_users(self, user, assignment_rule_doc, action="add"):
        ...
        if not user:
            dontmanage.throw(_("User Not found"))
            return
        ...
        if action == "add":
            ...
            base_assignment_rule = dontmanage.get_value(
                "HD Settings", "HD Settings", "base_support_rotation"
            )
            base_assignment_rule = dontmanage.get_doc(
                "Assignment Rule", base_assignment_rule
            )
            user_id = dontmanage.get_value(
                "Assignment Rule User",
                {"user": _user, "parent": base_assignment_rule.name},
            )
            if user_id:
                dontmanage.delete_doc("Assignment Rule User", user_id)
        else:
            user_id = dontmanage.get_value(
                "Assignment Rule User",
            ...
            total_users_in_assignment_rule = dontmanage.db.count(
