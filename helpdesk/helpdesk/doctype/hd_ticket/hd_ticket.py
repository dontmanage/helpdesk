...
from dontmanage.core.page.permission_manager.permission_manager import remove
...
from dontmanage.desk.form.assign_to import get as get_assignees
...
class HDTicket(Document):
    ...
    def notify_agent(self, agent, notification_type="Assignment"):
        dontmanage.get_doc(
            dontmanage._dict(
                ...
                user_from=dontmanage.session.user,
    ...
    def set_raised_by(self):
        self.raised_by = self.raised_by or dontmanage.session.user
    ...
    def set_priority(self):
        if self.priority:
            return
        self.priority = (
            dontmanage.get_cached_value("HD Ticket Type", self.ticket_type, "priority")
            or dontmanage.get_cached_value("HD Settings", "HD Settings", "default_priority")
    ...
    def set_first_responded_on(self):
        if self.status == "Replied":
            self.first_responded_on = (
                self.first_responded_on or dontmanage.utils.now_datetime()
            )

    def set_feedback_values(self):
        if not self.feedback:
            return
        feedback_option = dontmanage.get_doc("HD Ticket Feedback Option", self.feedback)
    ...
    def validate_feedback(self):
        if (
            ...
            return
        dontmanage.throw(
            _("Ticket must be resolved with a feedback"), dontmanage.ValidationError
        )
    ...
    def users_present_in_team_assignment_rule(self):
        ...
        assignment_rule = dontmanage.db.get_value(
            "HD Team", self.agent_group, "assignment_rule"
        )
        if not assignment_rule:
            return False

        is_disabled = dontmanage.db.get_value(
    ...
    def assign_agent(self, agent):
        ...
        if dontmanage.session.user != agent:
    ...
    def get_assigned_agents(self):
        ...
        if len(assignees) > 0:
            ...
            return dontmanage.get_all("HD Agent", filters={"name": ["in", names]})
    ...
    def get_assigned_agent(self):
        ...
        if hasattr(self, "_assign") and self._assign:
            ...
            if len(assignees) > 0:
                ...
                exists = dontmanage.db.exists("HD Agent", assignees[0])
                if exists:
                    return assignees[0]
        ...
        if len(assignees) > 0:
            # TODO: temporary fix, remove this when only agents can be assigned to ticket
            exists = dontmanage.db.exists("HD Agent", assignees[0].owner)
            if exists:
                agent_doc = dontmanage.get_doc("HD Agent", assignees[0].owner)
                return agent_doc
    ...
    def new_comment(self, content: str, attachments: List[str] = []):
        if not is_agent():
            dontmanage.throw(
                _("You are not permitted to add a comment"), dontmanage.PermissionError
            )
        c = dontmanage.new_doc("HD Ticket Comment")
        c.commented_by = dontmanage.session.user
        c.content = content
        c.is_pinned = False
    ...
    def create_communication_via_contact(self, message, attachments=[]):
        ...
        dontmanage.qb.update(QBFile).set(QBFile.attached_to_name, c.name).set(
            QBFile.attached_to_doctype, "Communication"
        ).where(Criterion.any(condition_name)).run()

        # attach files to ticket
        file_urls = dontmanage.get_all(
    ...
    def add_view(self):
        d = dontmanage.new_doc("View Log")
        d.reference_doctype = "HD Ticket"
        d.reference_name = self.name
        d.viewed_by = dontmanage.session.user
    ...
    def get_escalation_rule(self):
        ...
        for i in range(len(filters)):
            try:
                ...
                rule = dontmanage.get_last_doc("HD Escalation Rule", filters=f)
                if rule:
                    return rule
            except Exception:
                pass
    ...
    def apply_sla(self):
        ...
        Apply SLA if set.
        """
        if sla := dontmanage.get_last_doc("HD Service Level Agreement", {"name": self.sla}):
            sla.apply(self)
    ...
    # is an external dependency. Refer `communication.py` of DontManage framework for more.
    ...
    def on_communication_update(self, c):
        ...
        if c.sent_or_received == "Sent":
            ...
            if dontmanage.db.get_single_value("HD Settings", "auto_update_status"):
                self.status = "Replied"
    ...
    def attach_file_with_doc(self, doctype, docname, file_url):
        file_doc = dontmanage.new_doc("File")
...
# https://dontmanageframework.com/docs/user/en/python-api/hooks#modify-list-query
def permission_query(user):
    ...
    for c in customer:
        res += " OR `tabHD Ticket`.customer={customer}".format(
            customer=dontmanage.db.escape(c)
        )
    return res
...
def close_tickets_after_n_days():
    if dontmanage.db.get_single_value("HD Settings", "auto_close_tickets") == 0:
        return

    days_threshold = dontmanage.db.get_single_value("HD Settings", "auto_close_after_days")

    tickets_to_close = dontmanage.get_list(
        "HD Ticket",
        filters=[
            ["status", "=", "Replied"],
            [
                "modified",
                "<",
                dontmanage.utils.add_days(dontmanage.utils.now_datetime(), -days_threshold),
            ],
        ],
        pluck="name",
    )
    ...
    for ticket in tickets_to_close:
        doc = dontmanage.get_doc("HD Ticket", ticket)
        doc.status = "Closed"
