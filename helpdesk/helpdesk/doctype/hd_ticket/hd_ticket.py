...
from dontmanage.database.database import Criterion, Query
from dontmanage.desk.form.assign_to import add as assign
from dontmanage.desk.form.assign_to import clear as clear_all_assignments
from dontmanage.email.inbox import link_communication_to_document
from dontmanage.model.document import Document
from dontmanage.model.mapper import get_mapped_doc
from dontmanage.query_builder import Case, DocType
from dontmanage.query_builder.functions import Count
from dontmanage.utils import date_diff, get_datetime, now_datetime, time_diff_in_seconds
...
class HDTicket(Document):
	...
	def filter_by_team(query: Query):
		...
		should_filter: str = (
			dontmanage.get_value("HD Settings", None, "restrict_tickets_by_agent_group")
			or "0"
		)
		...
		QBTicket = dontmanage.qb.DocType("HD Ticket")
		filters = {"user": user}
		teams = dontmanage.get_list("HD Team", filters=filters)
	...
	def validate(self):
		if not self.raised_by:
			self.raised_by = dontmanage.session.user
	...
	def remove_assignment_if_not_in_team(self):
		...
		if self.has_value_changed("agent_group") and self.status == "Open":
			...
			) and dontmanage.get_doc(
				"Assignment Rule",
				dontmanage.get_doc("HD Team", self.agent_group).assignment_rule,
			).users:
	...
	def update_priority_based_on_ticket_type(self):
		if self.ticket_type:
			ticket_type_doc = dontmanage.get_doc("HD Ticket Type", self.ticket_type)
	...
	def set_contact(self, email_id, save=False):
		...
		if email_id:
			if not self.contact:
				contact = dontmanage.db.get_value("Contact", {"email_id": email_id})
				if contact:
	...
	def create_communication(self):
		communication = dontmanage.new_doc("Communication")
	...
	def split_ticket(self, subject, communication_id):
		...
		dontmanage.get_doc(replicated_ticket).insert()

		# Replicate linked Communications
		# TODO: get all communications in timeline before this, and modify them to append them to new doc
		comm_to_split_from = dontmanage.get_doc("Communication", communication_id)
		...
		for communication in communications:
			doc = dontmanage.get_doc("Communication", communication.name)
		...
		dontmanage.get_doc(
			{
				...
				"content": (
					" - Split the Ticket from <a href='/app/Form/Ticket/{0}'>{1}</a>".format(
						self.name, dontmanage.bold(self.name)
					)
				),
			}
	...
	def assign_agent(self, agent):
		...
		agent_name = dontmanage.get_value("HD Agent", agent, "agent_name")
	...
	def get_assigned_agent(self):
		if self._assign:
			assignees = json.loads(self._assign)
			if len(assignees) > 0:
				agent_doc = dontmanage.get_doc("HD Agent", assignees[0])
				return agent_doc
		return None

	def on_trash(self):
		activities = dontmanage.db.get_all("HD Ticket Activity", {"ticket": self.name})
		for activity in activities:
			dontmanage.db.delete("HD Ticket Activity", activity)

	def verify_ticket_type(self):
		if self.ticket_type:
			return

		settings = dontmanage.get_doc("HD Settings")
		...
		if not self.ticket_type:
			dontmanage.throw(_("Ticket type is mandatory"))

	def skip_email_workflow(self):
		skip: str = dontmanage.get_value("HD Settings", None, "skip_email_workflow") or "0"

		return bool(int(skip))

	def instantly_send_email(self):
		check: str = (
			dontmanage.get_value("HD Settings", None, "instantly_send_email") or "0"
		)
	...
	def last_communication(self):
		...
		communication = dontmanage.get_last_doc(
			"Communication",
			filters=filters,
		)
	...
	def last_communication_email(self):
		...
		email_account = dontmanage.get_doc("Email Account", communication.email_account)
	...
	def dashboard_uri(self):
		root_uri = dontmanage.utils.get_url()
	...
	def reply_via_agent(
		...
		if recipients == "Administrator":
			admin_email = dontmanage.get_value("User", "Administrator", "email")
			recipients = admin_email

		communication = dontmanage.get_doc(
			{
				"bcc": bcc,
				"cc": cc,
		...
		for attachment in attachments:
			file_doc = dontmanage.get_doc("File", attachment)
		...
		if not sender_email:
			dontmanage.throw(_("Can not send email. No sender email set up!"))
	...
	def is_seen(self):
		seen = self._seen or ""
		return dontmanage.session.user in seen
...
def set_descritption_from_communication(doc, type):
	if doc.reference_doctype == "HD Ticket":
		ticket_doc = dontmanage.get_doc("HD Ticket", doc.reference_name)
...
def create_communication_via_contact(ticket, message, attachments=[]):
	ticket_doc = dontmanage.get_doc("HD Ticket", ticket)
...
def get_all_conversations(ticket):
	conversations = dontmanage.db.get_all(
		"Communication",
		filters={
			"reference_doctype": ["=", "HD Ticket"],
	...
	for conversation in conversations:
		if dontmanage.db.exists("HD Agent", conversation.sender):
			# user User details instead of Contact if the sender is an agent
			sender = dontmanage.get_doc("User", conversation.sender).__dict__
			...
			if len(contacts) > 0:
				sender = dontmanage.get_doc("Contact", contacts[0].parent)
			else:
				sender = dontmanage.get_last_doc(
...
def get_user_tickets(filters="{}", order_by="creation desc", impersonate=None):
	...
	filters["raised_by"] = ["=", dontmanage.session.user]

	if impersonate and dontmanage.db.exists("HD Agent", dontmanage.session.user):
		filters["raised_by"] = ["=", impersonate]

	tickets = dontmanage.get_all(
		"HD Ticket",
		filters=filters,
		order_by=order_by,
		fields=[
...
def get_ticket_list(
	...
	user = dontmanage.session.user
	contact = dontmanage.db.get_value("Contact", {"user": user}, "name")
...
def set_multiple_status(names, status):
	...
	for name in json.loads(names):
		dontmanage.db.set_value("HD Ticket", name, "status", status)
...
def auto_close_tickets():
	...
	auto_close_after_days = (
		dontmanage.db.get_value("HD Settings", "HD Settings", "close_ticket_after_days")
		or 7
	)

	tickets = dontmanage.db.sql(
	...
	for ticket in tickets:
		doc = dontmanage.get_doc("HD Ticket", ticket.get("name"))
		doc.status = "Closed"
...
def has_website_permission(doc, ptype, user, verbose=False):
	...
	# from dontmanageerp.controllers.website_list_for_contact import has_website_permission
...
def make_ticket_from_communication(communication, ignore_communication_links=False):
	...
	doc = dontmanage.get_doc("Communication", communication)
	ticket = dontmanage.get_doc(
		{
			"doctype": "HD Ticket",
			"subject": doc.subject,
...
def calculate_first_response_time(ticket, first_responded_on):
	...
	support_hours = dontmanage.get_cached_doc("SLA", ticket.sla).support_and_resolution
...
def get_working_hours(date, support_hours):
	if is_work_day(date, support_hours):
		weekday = dontmanage.utils.get_weekday(date)
...
def get_holidays(holiday_list_name):
	holiday_list = dontmanage.get_cached_doc("HD Service Holiday List", holiday_list_name)
