	...
	def validate(self):
		...
		for field in self.fields:
			...
			if field.fieldname == "description" and field.fieldtype != "Text Editor":
				dontmanage.throw(f"field type for description field should be Text Editor")
		...
		if len(required_fields_not_added) > 0:
			dontmanage.throw(f"template mandetory fields {required_fields_not_added} are not added")
...
def refresh_server_script():
	all_hd_ticket_templates = dontmanage.get_all("HD Ticket Template")
	...
	for template in all_hd_ticket_templates:
		template_doc = dontmanage.get_doc("HD Ticket Template", template)
		flag = False
	...
	server_script = dontmanage.get_doc("Server Script", "Ticket Auto Set Custom Fields")
