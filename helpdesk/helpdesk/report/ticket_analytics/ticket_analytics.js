...
dontmanage.query_reports["Ticket Analytics"] = {
  filters: [
    {
      fieldname: "based_on",
      label: __("Based On"),
      ...
      events: {
        onCheckRow: function (data) {
          if (data && data.length) {
            ...
            let raw_data = dontmanage.query_report.chart.data;
            ...
            setTimeout(() => {
              dontmanage.query_report.chart.update(new_data);
            }, 500);

            setTimeout(() => {
              dontmanage.query_report.chart.draw(true);
            }, 1000);
            dontmanage.query_report.raw_chart_data = new_data;
          }
        },
      },
    });
