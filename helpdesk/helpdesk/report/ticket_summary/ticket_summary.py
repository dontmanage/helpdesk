...
class TicketSummary(object):
	...
	def get_summary_data(self):
		self.ticket_summary_data = dontmanage._dict()
			...
			if self.filters.based_on == "Assigned To":
				if d._assign:
					for entry in json.loads(d._assign):
						self.ticket_summary_data.setdefault(entry, dontmanage._dict()).setdefault(status, 0.0)
						self.ticket_summary_data.setdefault(entry, dontmanage._dict()).setdefault(
				...
				self.ticket_summary_data.setdefault(value, dontmanage._dict()).setdefault(status, 0.0)
				self.ticket_summary_data.setdefault(value, dontmanage._dict()).setdefault(
					agreement_status, 0.0
				)
				self.ticket_summary_data.setdefault(value, dontmanage._dict()).setdefault(
					"total_tickets", 0.0
	...
	def get_metrics_data(self):
		...
		if ticket:
			if self.filters.based_on == "Assigned To":
				assignment_map = dontmanage._dict()
				for d in self.entries:
					if d._assign:
						for entry in json.loads(d._assign):
				...
				for entry in data:
					...
					for metric in metrics_list:
						self.ticket_summary_data.setdefault(value, dontmanage._dict()).setdefault(metric, 0.0)
