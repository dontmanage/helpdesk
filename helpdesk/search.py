...
from dontmanage.utils import cstr, strip_html_tags, update_progress_bar
...
@redis_cache(3600 * 24)
def get_stopwords():
    return STOPWORDS + dontmanage.get_all("HD Stopword", {"enabled": True}, pluck="name")


@redis_cache(1800)
def get_synonym_words() -> list[str]:
    ret = dontmanage.get_all("HD Synonym", ["name"], as_list=True) + dontmanage.get_all(
...
class Search:
    ...
    def __init__(self, index_name, prefix, schema) -> None:
        self.redis = dontmanage.cache()
        self.index_name = index_name
        self.prefix = prefix
        self.schema = []
        for field in schema:
            self.schema.append(dontmanage._dict(field))
    ...
    def add_synonyms(self):
        for word, synonym in dontmanage.get_all(
    ...
    def search(
        ...
        try:
            ...
            print(e)
            return dontmanage._dict({"total": 0, "docs": [], "duration": 0})

        out = dontmanage._dict(docs=[], total=result.total, duration=result.duration)
        for doc in result.docs:
            id = doc.id.split(":", 1)[1]
            _doc = dontmanage._dict(doc.__dict__)
            _doc.id = id
...
class HelpdeskSearch(Search):
    ...
    def __init__(self):
        settings: "HDSettings" = dontmanage.get_cached_doc("HD Settings")
        schema = [
    ...
    def build_index(self):
        ...
        for i, doc in enumerate(records):
            self.index_doc(doc)
            if not hasattr(dontmanage.local, "request"):
    ...
    def get_count(self, doctype):
        if doctype == "HD Ticket":
            return dontmanage.db.count(doctype)
    ...
    def get_records(self, doctype):
        ...
        for d in dontmanage.db.get_all(
...
def build_index():
    dontmanage.cache().set_value("helpdesk_search_indexing_in_progress", True)
    search = HelpdeskSearch()
    search.build_index()
    dontmanage.cache().set_value("helpdesk_search_indexing_in_progress", False)


def build_index_in_background():
    if not dontmanage.cache().get_value("helpdesk_search_indexing_in_progress"):
        dontmanage.enqueue(build_index, queue="long")
