...
def add_default_categories_and_articles():
    category = dontmanage.db.exists("HD Article Category", DEFAULT_ARTICLE_CATEGORY)
    if not category:
...
def add_default_sla():
    ...
    if dontmanage.db.exists("HD Service Level Agreement", "Default"):
        return
...
def add_default_holiday_list():
    if dontmanage.db.exists("HD Service Holiday List", "Default"):
        return
    dontmanage.get_doc(
        {
...
def add_property_setter():
    if not dontmanage.db.exists("Property Setter", {"name": "Contact-main-search_fields"}):
        doc = dontmanage.new_doc("Property Setter")
